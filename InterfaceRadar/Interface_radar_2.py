...
class MyWindow(tk.Tk): 
    ...
    def __init__(self):
        ...
        largeur_ecran = largeur_ecran-250
        hauteur_ecran = hauteur_ecran-100
        print(largeur_ecran)
        print(hauteur_ecran)
        ...
        left_canvas_height = 210
        top_left_canvas_width = 425
        middle_column_width = 330
        right_canvas_width = largeur_ecran - (top_left_canvas_width+middle_column_width+25)
        left_canvas_last_height = hauteur_ecran-649
        ...
        self.canvas_left_1 = tk.Canvas(self, borderwidth =5, relief="ridge",bg="white", width=top_left_canvas_width, height=left_canvas_height)
        self.canvas_left_1.place(x=0, y=0)
        self.canvas_left_2 = tk.Canvas(self, borderwidth =5, relief="ridge", bg="white", width=top_left_canvas_width, height=left_canvas_height)
        self.canvas_left_2.place(x=0, y=left_canvas_height+6)

        # Canevas pour les deux premières lignes dans la colonne du milieu
        self.canvas_middle_1 = tk.Canvas(self, borderwidth =5, relief="ridge", bg="white", width=middle_column_width, height=top_left_canvas_width+1)
        self.canvas_middle_1.place(x=top_left_canvas_width+6, y=0)
        print(middle_column_width-50)
        print(2*left_canvas_height)
        # Canevas pour les deux dernières lignes (colonnes 1 et 2 combinées)
        self.canvas_combined_1 = tk.Canvas(self, bd=5, relief="ridge", bg="white", width=760, height=left_canvas_height)
        self.canvas_combined_1.place(x=0, y=top_left_canvas_width+8)
        self.canvas_combined_2 = tk.Canvas(self, bd=5, relief="ridge", bg="white", width=773, height=left_canvas_last_height-83)
        self.canvas_combined_2.place(x=0, y=649)

        # Canevas pour la colonne de droite
        self.audio_canvas = tk.Canvas(self, bd=5, relief="ridge", bg="white", width=right_canvas_width, height=right_canvas1_height)
        self.audio_canvas.place(x=767, y=0)

        self.spectrogram_canvas = tk.Canvas(self, bd=5, relief="ridge", bg="white", width=right_canvas_width, height=right_canvas2_height-90)
        self.spectrogram_canvas.place(x=767, y=right_canvas1_height+6)
        ...
        self.channels = {}
        self._widgets_gbf("1",420,25,color_voie="gold",def_fonc="DC",def_freq=200,def_volt=4.5,def_offset=3.5,def_load="INF",def_phase=0,def_etat="0")
        ...
        self.default_params = {
            0: {  # Mode Doppler
                "1": {"func": "DC", "freq": 200, "volt": 4.5, "offset": 3.5, "load": "INF", "phase": 0, "etat": 0},
            ...
            1: {  # Mode Synchro
                ...
                "2": {"func": "PULS", "freq": 200, "volt": 4.5, "offset": 0, "load": "INF", "phase": 176, "etat": 0},
            },
        }
        # Paramètres actuels
        ...
        self.bouton_audio = tk.Button(self, text="Run Audio", bg='thistle', command=self.run_audio_thread)
        self.bouton_audio.place(x=350, y=395)
        ...
        self.combobox_color = ttk.Combobox(self, width=10, values=['plasma','viridis','inferno','magma','cividis','hsv','Spectral'])
        self.combobox_color.place(x=410, y=595)
        self.combobox_color.set('plasma')
        ...
        self.label_color = tk.Label(self, text=f"Couleur Spectrogramme : ", bg="white")
        self.label_color.place(x=260, y=595)
        ...
        self.bouton_selection_color.place(x=260, y=615)

        #-----------Creation du bouton Run spectrogramme----------------
        self.bouton_spectrogram = tk.Button(self, text="Run Spectrogram", bg='thistle', command=self.set_tpc_to_0_and_run)
        self.bouton_spectrogram.place(x=260, y=460)
        self.bouton_tpc = tk.Button(self, text="Two Pulse Cancel", bg='thistle', command=self.set_tpc_to_1_and_run)
        self.bouton_tpc.place(x=260, y=510)

        #-----------Creation d'un entry qui permet de modifier le nombre de ligne pour Two Pulse Cancel----------------
        self.value_ligne = tk.IntVar()
        self.value_ligne.set(N_pixel_vertical)  # Valeur par défaut de zpad
        self.texte_ligne = tk.Label(self, text="Moyenne sur :               lignes (impaires)", bg="white")
        self.texte_ligne.place(x=365, y=510)
        self.entry_ligne = tk.Spinbox(self, from_=1, to=251,increment=2, width=4, textvariable=self.value_ligne)
        self.entry_ligne.place(x=446, y=512)

        self.value_min = tk.IntVar()
    ...
    def on_resize(self, event):
            largeur_ecran = self.winfo_width()  # Nouvelle largeur de la fenêtre
            hauteur_ecran = self.winfo_height()  # (Nouvelle hauteur de la fenêtre
            
            # Recalculer la largeur et la hauteur du canevas
            right_canvas_width = max(largeur_ecran - 766, 100)  # Évitez les valeurs trop petites
            right_canvas1_height = max(300, 100)
            right_canvas2_height = max(hauteur_ecran-right_canvas1_height+7, 100)
            left_canvas_heigt = max(hauteur_ecran-649, 100)
            # Ajuster la taille et la position du canevas
            self.audio_canvas.place(x=767, y=0, width=right_canvas_width, height=right_canvas1_height)
            self.spectrogram_canvas.place(x=767, y=right_canvas1_height-7,width=right_canvas_width, height=right_canvas2_height)
            self.canvas_combined_2.place(x=0, y=649,width=774, height=left_canvas_heigt)
    ...
    def transmission_param(self):
        ...
        try:
            ...
            volt_voie1 = self.channels["1"]["value_volt"].get()/2
            offset_voie1 = self.channels["1"]["value_offset"].get()/2
            ...
            volt_voie2 = self.channels["2"]["value_volt"].get()/2
            offset_voie2 = self.channels["2"]["value_offset"].get() /2
    ...
    def rechercher_audio(self):
        ...
        if fichier:  # Si un fichier est sélectionné
            try:
                ...
                audio_dir = os.path.join(script_dir, "audio_wav")
                chemin_destination = os.path.join(audio_dir, nom_fichier)
    ...
    def entry_zpad_changed(self):
        global zpad_doppler, zpad_synchro, tp_doppler, tp_synchro, mode
        if mode==0:
...
def spectrogramme_synchro(audio, callback, queue_manager):
    ...
    global data, zpad_synchro, tp_synchro, cmap, tpc, N_pixel_vertical, V_min, V_max
    try:
        queue_manager.put(("progress", "Lecture du fichier audio"))
        fs, Y = wavfile.read(os.path.join(os.path.dirname(os.path.abspath(__file__)), "audio_wav", audio))
        ...
        if tpc==1 :
            try:
                N_pixel = N_pixel_vertical#100#200
                Nb_ligne = np.matrix(S)
                Nb_ligne = Nb_ligne.shape
                result = []
                for i in range(0, Nb_ligne[0], N_pixel):
                    block = S[i:i + N_pixel]  # Sélectionner un bloc de N_pixel lignes
                    block_mean = np.mean(block, axis=0)  # Calculer la moyenne par colonne
                    result.append(block_mean.tolist())  # Ajouter le résultat comme une liste
                result_array = np.array(result)    
                Nb_ligne_res = np.matrix(result_array)
                Nb_ligne_res = Nb_ligne_res.shape

                diff = np.abs(result_array[:-1] - result_array[1:])
                m_diff = np.max(diff)

                plt.figure(figsize=(10, 6))
                plt.imshow(diff - m_diff, aspect='auto', extent=[0, max_range, Nb_ligne_res[0], time[0]],vmin=Vmin,vmax=Vmax, cmap=cmap)
                plt.colorbar()
                plt.ylabel('Temps (s)')
                plt.xlabel('Distance (m)')
                plt.title('Différence')
                print("spectrogramme with two pulse cancel")

                spec_name = 'spectre_synchro_tpc.png'

            except Exception as e:
                print(f"Erreur dans le bloc TPC : {e}")
                plt.close()
